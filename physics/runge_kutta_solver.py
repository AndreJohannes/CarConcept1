...
class Solver:
	...
	def solve_step(self, state):
		...
		k2, s2 = system.get_function(numpy.add(state,self.step_size/2.0*k1))
		k3, s3 = system.get_function(numpy.add(state,self.step_size/2.0*k2))
		k4, s4 = system.get_function(numpy.add(state,self.step_size*k3))
		return state + self.step_size / 6.0 * ( k1+2*k2+2*k3+k4 ), (s1 + 2*s2 +  2*s3 + s4) / 6.0